...
describe("ProfileStatus component", () => {
    test("status from props should be in the state", () => {
        const component = create(<ProfileStatus status="hi hi"/>);
        const instance = component.getInstance();
        expect(instance.state.status).toBe("hi hi");
    });
    ...
    test("after creation span with status should contains correct status", () => {
        ...
        const root = component.root;
        let span = root.findByType("span");
        expect(span.children[0]).toBe("hi hi");
    });
    test("input should be displayed in editMode istead of span", () => {
        ...
        let input = root.findByType("input");
        expect(input.props.value).toBe("hi hi");
    });
    test("callback should be called", () => {
        const mockCallback = jest.fn()
        const component = create(<ProfileStatus status="hi hi" updateUserStatus={mockCallback}/>);
        const instance = component.getInstance();
